...
class Vida:
	def __init__(self,enemyx,enemyy):
		...
		self.negro = (0,0,0)
		self.updateVida(self.z,self.cont,enemyx,enemyy)
	...
	def MensajePantalla(Mensaje , color , cambioy = 0):
		textoSuperficie , textoRect = self.ObjetoTexto
	...
	def updateVida(self,playerx,playery,Enemyx1,Enemyy1):
		self.Variacionx = (playerx - Enemyx1)**2
		self.Variaciony = (playery - Enemyy1)**2
		self.cont+=1
		if(self.cont == 600):
		...
		if(self.Variacionx < 9 and self.Variaciony < 9):
			...
			while self.gameover == True:
				...
				if(self.contDie == 8):
					self.MensajePantalla("Has perdido" , self.gris1,20)
				if(self.contDie == 9):
					self.MensajePantalla("Has perdido" , self.gris2,20)
				if(self.contDie == 10):
					self.MensajePantalla("Has perdido" , self.negro,20)
					self.MensajePantalla("ESPACIO para continuar y ESCAPE para salir" , self.negro , 0)
...
def Juego():
	...
	player1 = player()
	enemy1 = Enemy()
	vida1 = Vida(enemy1.x1,enemy1.y1)
	position = 0
	pygame.mixer.init()
	...
	while 1:
	    ...
	    vida1.updateVida(player1.x,player1.y,enemy1.x1,enemy1.y1)
	    pygame.display.flip()       
Juego()
